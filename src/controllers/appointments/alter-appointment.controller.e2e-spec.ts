...
describe('Alter appointment (E2E)', () => {
  let app: INestApplication
  let prisma: PrismaService
  let jwt: JwtService
  let atendenteToken: string
  let profissionalOneToken: string
  ...
  test('[PUT] /appointments/alter - Profissional - Fail', async () => {
  ...
  test('[PUT] /appointments/alter - Profissional - Success', async () => {
    const response = await request(app.getHttpServer())
      .put('/appointments/alter')
      .set('Authorization', `Bearer ${profissionalTwoToken}`)
      .send({
        id: 2,
        start: '2024-10-17T07:51:00Z',
        end: '2024-10-17T08:40:00Z',
      })

    const appAltered = await prisma.appointment.findUnique({ where: { id: 2 } })
    expect(response.statusCode).toBe(201)
    expect(appAltered?.start.getTime()).toEqual(1729151460000)
  })

  test('[PUT] /appointments/alter - Atendente - Fail', async () => {
    const response = await request(app.getHttpServer())
      .put('/appointments/alter')
  ...
  test('[PUT] /appointments/alter - Atendente - Fail', async () => {
    const response = await request(app.getHttpServer())
      .put('/appointments/alter')
      .set('Authorization', `Bearer ${profissionalOneToken}`)
      .send({
        id: 3,
        start: '2024-10-24T07:00:00Z',
        end: '2024-10-24T07:50:00Z',
